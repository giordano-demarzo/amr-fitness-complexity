...
pathogen--antibiotic pairs become resistant in a rolling-origin test over 2020--2024 (AUC $0.67$,
...
consistently. Bugs in the most-exposed group are about \emph{four times} more likely to acquire the
new resistance than those in the least-exposed group ($15\%$ vs.\ $3.6\%$; Fig.~\ref{fig:pred}b). Its
overall accuracy, measured as the probability of ranking a true future resistance above a non-event
(the AUC), is $0.67$ for the drug network and $0.64$ for the analogous pathogen network (two species
linked when they resist similar drugs), stable across every year tested ($0.63$--$0.77$ and
$0.61$--$0.68$). To be sure this reflects the real wiring and not just how busy each node is, we
reshuffled the network labels \num{200} times: the true score stands far above this chance baseline
($z=4.4$ and $8.0$, $p<0.01$).
...
(a) Antibiotic co-resistance network (NBI projection), nodes coloured by WHO AWaRe class, sized by
...
\paragraph{Projections.} One-mode projections use the resource-allocation (NBI) weighting of
Ref.~\cite{zhou2007}: $W_{ab}=\tfrac{1}{k_b}\sum_p \tfrac{M_{pa}M_{pb}}{k_p}$ for the antibiotic
layer (and the transpose for pathogens), symmetrised. We also verified that a bipartite-configuration
(BiCM) validated projection yields essentially no edges, consistent with co-resistance being
degree-explained; NBI and raw co-occurrence give equivalent predictive performance.
...
projection (NBI) are both fixed a priori, and the exposure score is a pure ranking. For each origin
...
binarizations (absolute thresholds, $\rca\ge1.5$) and network definitions (raw co-occurrence,
BiCM-validated) confirms the a-priori choice is representative rather than optimised.
...
\bibitem{zhou2007} T. Zhou \textit{et al.}, \textit{Phys. Rev. E} \textbf{76}, 046115 (2007).
